...
def check_routing(state: State) -> str:
    """Quyết định gọi agent nào dựa trên nội dung câu hỏi.
...
def privacy_agent(state: State) -> dict:
    ...
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia bảo mật dữ liệu và GDPR. Phân tích khía cạnh bảo mật trong câu hỏi:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR, quyền riêng tư, vi phạm dữ liệu, nghĩa vụ báo cáo vi phạm, biện pháp khắc phục."""
    response = llm.invoke([HumanMessage(content=prompt)])
...
def build_graph() -> StateGraph:
    ...
    # Conditional routing based on the node name returned by check_routing
    graph.add_conditional_edges("check_routing", lambda state: check_routing(state))
